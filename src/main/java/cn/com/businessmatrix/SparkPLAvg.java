...
public class SparkPLAvg {
    ...
    public static void main(String[] args) throws Exception {
		...
		String step="1";//第1步骤，先处理出的，第2步骤，再处理入，因为入的成本要等出的成本		
...

        {
            ...
            if ("1".equals(step)) {
                ...
                sql_lookdf = " (  select cast(fnv_hash("
                        + "concat("
			...
			*/
            ...
            if ("1".equals(step)) {
                sql = " (  select cast(fnv_hash("
                        ...
                        "               ,t.confirm_balance " +
                        "               ,t.confirm_shares " +
                        "               ,nvl(t3.back_fee,0) +t.total_fee-t.back_fee as total_fee " + //补差费从转出方转移到转入方
                        "               ,t.net_value " +
                        "               ,t.requestno " +
                        "               ,t.dk_bonus_type " +
                        "               ,t.dk_bourseflag" +
                        ...
                        + " on t.ta_cfm_date=t3.ta_cfm_date and t.requestno=t3.requestno and t.agencyno=t3.agencyno and t3.sk_mkt_trade_type<>t.sk_mkt_trade_type " +
                        " where t.ta_cfm_date between " + startdate + " and " + enddate +
                        " and t.dk_system_of_upd='" + srcsys + "' " +
    ...
    static Row changeEt(Row currRow, Row tranRow, StructType st, double[] params) {
        ...
        Row ret = new GenericRowWithSchema(new Object[]{
                ...
                tranRow.getAs("dk_system_of_upd"),//null,33
    ...
    static Row changeEt2(Row tranRow, StructType st, double[] params, List<String> transferTypesList) {
        ...
        Row ret = new GenericRowWithSchema(new Object[]{
                ...
                sdf.format(new java.sql.Timestamp(System.currentTimeMillis())), //31
                sdf.format(new java.sql.Timestamp(System.currentTimeMillis())), //32
                tranRow.getAs("dk_system_of_upd"),//33
                tranRow.getAs("ta_cfm_date"), //effective_from 34,
                tranRow.getAs("dk_system_of_upd")//dk_system_of_sdata 35
        }, st
        );
        return ret;
    }

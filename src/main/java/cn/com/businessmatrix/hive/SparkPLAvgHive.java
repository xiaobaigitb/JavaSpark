 ...
 *  1.2找出没有做过第1步骤交易的账户。
 *  1.3从落地表（ass_ta_cust_income_avgcost,ass_ta_cust_income_avgcost_et)里面取出最后一条记录(99991231),
 ...
 *  1.7把spark的数据合并到ass_ta_cust_income_avgcost,ass_ta_cust_income_avgcost_et。
 *     此时出的交易都处理完成。
 *     
 *  步骤二：因为转托管出的平均成本已经计算好，开始处理"入"的账户交易。
 *  2.1找出做过托管入（内部转托管入、非交易过户入等）交易的账户。 
 *  2.2找出做过托管出（内部转托管出、非交易过户出等）交易时刻的平均成本。
 *  2.3从落地表（ass_ta_cust_income_avgcost,ass_ta_cust_income_avgcost_et)里面取出最后一条记录(99991231),
 ...
 *  2.7把spark的数据合并到ass_ta_cust_income_avgcost,ass_ta_cust_income_avgcost_et。
 *     此时"入"的交易都处理完成。
 *  2.8稽核数据。  
 *     
 *  参数顺序：
...
public class SparkPLAvgHive {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String postfix = "_byday";
        String savemode = "append";
        if (args.length > 8)
            savemode = args[8];

        String transferInTypes = "(127,98)";//args[7];//转托管入，非交易过户入
        String transferOutTypes = "(128,99)";//args[8];//转托管出，非交易过户出
		...
		String sparkname="ass_ta_cust_income_avgcost_88888888";//spark任务名称，方便查找
		...
		String schema="ods";
		String sparktable="tmp_ass_ta_cust_income_avgcost_spark";
		List<String> transferTypesList= new ArrayList(Arrays.asList(transferInTypes.replace("(", "").replace(")","").split(",")));
        transferTypesList.addAll(Arrays.asList(transferOutTypes.replace("(", "").replace(")","").split(",")));
		*/
...

        {
            ...
            if ("1".equals(step)) {
                ...
                sql_lookdf = "  select  concat_ws(',',"
                        ...
                        + "cast(t1.sk_product       as string))  as hashcode, " + //0 份额类型在博时没有意义
                        ...
                        "  from " + schema + ".ass_ta_cust_income_avgcost" + postfix + "  t1 inner join " +
                        schema + ".ass_ta_cust_income_avgcost_et" + postfix + "  t2 on t1.trd_it_trx_serialno=t2.trd_it_trx_serialno " +
                        " inner join  " + schema + ".tmp_ass_ta_cust_income_avgcost_notin" + postfix + "  t  on t1.sk_account_of_fd = t.sk_account_of_fd " +
                        " and t1.sk_tradeacco_reg = t.sk_tradeacco_reg " +
                        " and t1.sk_product       = t.sk_product " +
                        //" and t1.dk_share_type    = t.dk_share_type "+
                        " left join   " + schema + ".tmp_ass_ta_cust_income_avgcost_in" + postfix + "  t3 on  t1.sk_account_of_fd = t3.sk_account_of_fd " +
                ...
                sql_lookdf = "   select  concat_ws(',',"
                        + "cast(t1.sk_account_of_fd as string),"
                        + "cast(t1.sk_tradeacco_reg as string),"
                        + "cast(t1.sk_product       as string)) as hashcode, " + //0
                        "       t1.trd_it_trx_serialno," + //1
                        "       t1.dk_tano," + //2
                        "       t1.cserialno, " + //3
                        "       t1.requestno, " +  //4
                        ...
                        "  from " + schema + ".ass_ta_cust_income_avgcost" + postfix + "  t1 inner join " +
                        schema + ".ass_ta_cust_income_avgcost_et" + postfix + "  t2 on t1.trd_it_trx_serialno=t2.trd_it_trx_serialno " +
                        " inner join   " + schema + ".tmp_ass_ta_cust_income_avgcost_in" + postfix + "  t3 on  t1.sk_account_of_fd = t3.sk_account_of_fd " +
                        " and t1.sk_tradeacco_reg = t3.sk_tradeacco_reg " +
                        " and t1.sk_product 	  = t3.sk_product " +
                        //" and t1.dk_share_type    = t3.dk_share_type "+
                        " where t1.dk_system_of_sdata='" + srcsys + "'  and t2.effective_to=99991231 " +
            ...
            if ("1".equals(step)) {
                ...
                sql = "  select  concat_ws(',',"
                        ...
                        + "cast(t.sk_product       as string))  as hashcode " +
                        "               ,t.sk_invpty_of_cust " +
                        "               ,t.dk_cust_type " +
                        "               ,t.sk_account_of_fd " +
                        ...
                        " inner join   " + schema + ".tmp_ass_ta_cust_income_avgcost_notin t1 on t1.sk_account_of_fd = t.sk_account_of_fd " +
                        " and t1.sk_tradeacco_reg = t.sk_tradeacco_reg " +
                        " and t1.sk_product       = t.sk_product " +
                        " and t1.dk_share_type    = t.dk_share_type " +
                        " left join   " + schema + ".tmp_ass_ta_cust_income_avgcost_in t2 on  t2.sk_account_of_fd = t.sk_account_of_fd " +
                        " and t2.sk_tradeacco_reg = t.sk_tradeacco_reg " +
                        " and t2.sk_product       = t.sk_product " +
                        " and t2.dk_share_type    = t.dk_share_type " +
                        " left join   " + schema + ".tmp_ass_ta_cust_income_avgcost_change t3 "
                        + " on t.ta_cfm_date=t3.ta_cfm_date and t.requestno=t3.requestno and t.agencyno=t3.agencyno  and t.sk_mkt_trade_type <> 137 " +
                ...
                sql = "  select concat_ws(',',"
                        ...
                        " left join  " + schema + ".prod_product pp on t.sk_product=pp.sk_product " +
                        " left join  " + schema + ".agrm_tradeaccomap tap on t.sk_tradeacco_reg=tap.sk_tradeacco_reg_of_out and t.dk_tano=tap.dk_tano " +
                        " inner join   " + schema + ".tmp_ass_ta_cust_income_avgcost_in t1 on t1.sk_account_of_fd = t.sk_account_of_fd " +
                        " and t1.sk_tradeacco_reg = t.sk_tradeacco_reg " +
                        " and t1.sk_product       = t.sk_product " +
                        " and t1.dk_share_type    = t.dk_share_type " +
                        " left join    " + schema + ".tmp_ass_ta_cust_income_avgcost_out  t2 on  t2.sk_product       = t.sk_product " +
                        " and t2.dk_share_type    = t.dk_share_type " +
    ...
    static Row changeEt(Row currRow, Row tranRow, StructType st, double[] params) {
        ...
        //System.out.println("613="+params[1]);
        ...
        //System.out.println("618="+currRow.getAs(st.fieldIndex("avg_cost")));
    ...
    static Row changeEt2(Row tranRow, StructType st, double[] params, List<String> transferTypesList) {
        ...
        //System.out.println("668="+params[1]);
        ...
        if (shr_chg_dir == 1)//增加份额
        {
            if (sk_mkt_trade_type == 143)//分红按增加份额处理，但是收益规则处理写死
            {
                ...
                //System.out.println("734="+new_avg_cost);
        ...
        } else if (shr_chg_dir == -1)//减少份额
        {
            ...
            new_shares = shares - tr_shares;
            new_avg_cost = avg_cost;
            //System.out.println("769="+new_avg_cost);
        }

        //System.out.println("772="+Double.isInfinite(new_avg_cost));
        //System.out.println("773="+Double.isFinite(new_avg_cost));
        //System.out.println("774="+Double.isNaN(new_avg_cost));
        ...
        //System.out.println("797="+new_avg_cost);
        //System.out.println("798="+Double.parseDouble(new DecimalFormat("#.00000000").format(new_avg_cost)));
        ...
        Row ret = new GenericRowWithSchema(new Object[]{
                ...
                new BigDecimal(new_total_cost - total_cost), //total_cost 30
                null,//dk_is_valid 31

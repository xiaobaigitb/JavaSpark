 ...
 *
 */
public class SparkFltAssCustFundbalNonmnyHive {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String vn_startdate = hc.sql("select t.last_wkdate from " + schema_pub + ".flt_comm_cnexchg_date t where t.sk_date=" + startdate)
		    ...
		    /* 优化，其实ass_ta_fundbal_chg基本要全表扫描
        ...
        hc.sql("alter table " + schema_pub + "." + sparktable + " drop if exists partition(dk_is_momney='0')");
        System.out.println("alter table " + schema_pub + "." + sparktable + " drop if exists partition(dk_is_momney='0')");
        ...
        String sql_lookdf = "SELECT sk_invpty_of_cust ,dk_cust_type ,cust_name ,dk_id_type ,std_idno ,sk_region ,bk_region ,region_name ,sk_account_of_fd ,bk_fundaccount "
                ...
                + ",frozen_income ,lc_frozen_income ,capital ,lc_capital ,all_capital ,lc_all_capital ,dk_system_of_sdata ,dk_system_of_upd ,nav_date ,sk_date_of_pre,bk_invpty_of_cust, dk_is_momney"
        ...
        String sql = " ( SELECT shr_chg_serialno ,"
                ...
                + " ,memo ,ddvc ,dk_system_of_upd ,batchno ,inserttime ,updatetime ,dk_system_of_sdata ,effective_from_m "
                + " FROM " + schema_ods + ".ass_ta_fundbal_chg t "
                + " WHERE dk_system_of_sdata='" + srcsys + "' "
                //+ " t.effective_from_m  BETWEEN  "+ vn_startmth +" AND  " + vn_endmth
                + " AND t.shares<>0 )";
        ...
        sql = " SELECT /*+ BROADCAST(t1) */ t.sk_invpty_of_cust AS sk_invpty_of_cust ," +
                ...
                " 	WHERE t1.bus_date " +
                " 		BETWEEN least(" + vn_startdate + ",cast(concat(substr(" + startdate + ",1,6),'01') AS int)) " +
                " 			AND cast(concat(substr(" + enddate + ",1,6),'31') AS int) " +
                " 			AND " +
                " 		CASE " +

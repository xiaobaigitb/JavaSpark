...
/* 国海个性化调整,适配赎回按单拆分：effective_to=effective_from */
public class SparkAreaSplit3_1 {

    /*
     * 管理区划拆分-国海模式，保有表从盈亏推出，支持ta流水号拆分
     */
    ...
    public static void main(String[] args) {
		...
		*/
        ...
        //dBConOption.put("useKeyTab", "true");
        //dBConOption.put("keyTab", "/home/etl/impala.keytab");
        //dBConOption.put("principal", "impala/cdh04.businessmatrix.com.cn@businessmatrix.com.cn");
        //dBConOption.put("doNotPrompt", "true");
        ...
        String sql_schema =
                "( SELECT t.shr_chg_serialno,\n" +
                        ...
                        "  FROM " + schema + ".ass_ta_shrdt_arsplt t\n" +
                        " WHERE 1 = 2) as tt"; //Modified By Luogong,20220520: 修改表名为ass_ta_shrdt_arsplt
        ...
        String sql_data =
                " (SELECT t.shr_chg_serialno,\n" +
                        ...
                        "       t.dk_system_of_sdata,\n" +
                        "       t.dk_share_type\n" +
                        "  FROM  " + schema + ".ass_ta_saletran_fundbal_chg   t\n" + //ass_ta_fundbal_chg
            ...
            public List<Row> call(Row r) throws Exception {
                ...
                while (it.hasNext()) {
                    ...
                    for (int i = 0; i < vt_dt_range.size(); i++) {
                        ...
                        //只取有效日期区间段
                        if (v_ef <= v_et && v_accu_ratio > v_e) //EF=ET:适配赎回按单拆分 v_ef<v_et
                        {
                            //按其它拆分维度匹配规则
                            if (
                    ...
                    if (vt_dt_range_split.size() > 0) {
                        ...
                        v_dtrg_no = 0; //EF=ET:适配赎回按单拆分，初始化计数器变量
                        vt_dt_range_pre.add(new DateRange(19000101, 0, 0.0d)); //EF=ET:适配赎回按单拆分，初始化标号为0的值
                        ...
                        for (v_cnt1 = 0; v_cnt1 < v_count1; v_cnt1++) {
                            ...
                            //重新构造日期区间
                            if (v_cnt1 >= 1 && vt_array_seq.get(v_cnt1) >= vt_array_seq.get(v_cnt1 - 1)//EF=ET: 适配赎回按单拆分
                                    && vt_array_seq.get(v_cnt1) != vt_dt_range_pre.get(v_dtrg_no).start_dt //EF=ET: 适配赎回按单拆分，不等于新数组上一条起始日期
                            ) {
                                vt_dt_range_pre.add(new DateRange(vt_array_seq.get(v_cnt1 - 1), vt_array_seq.get(v_cnt1), 0.0d));
                                v_dtrg_no = v_dtrg_no + 1;//EF=ET: 适配赎回按单拆分，计数器累计
                            }
                        } //end for v_cnt1

                        vt_dt_range_pre.remove(0);//EF=ET: 适配赎回按单拆分，清除初始化数组值
                        ...
                        for (v_cnt1 = 0; v_cnt1 < vt_dt_range_pre.size(); v_cnt1++) {
                            ...
                            for (v_cnt2 = 0; v_cnt2 < vt_dt_range_split.size(); v_cnt2++) {
                                if (vt_dt_range_pre.get(v_cnt1).start_dt >= vt_dt_range_split.get(v_cnt2).start_dt
                                        && vt_dt_range_pre.get(v_cnt1).start_dt <= vt_dt_range_split.get(v_cnt2).end_dt ////EF=ET:适配赎回按单拆分 国海< -> <=
                                ) {

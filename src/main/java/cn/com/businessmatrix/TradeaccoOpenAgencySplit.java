...
public class TradeaccoOpenAgencySplit {
    public static void main(String[] args) {
        ...
        String pi_schema = args[1];
        String pi_batchno = args[2];
        String pi_dsagencyno = args[3];
        String pi_tgtTableName = args[4];
        String pi_isAll = args[5];
        if (pi_isAll.isEmpty())
            pi_isAll = "2";

        String sql_data = "SELECT t.sk_tradeacco_reg       ,t.agencyno AS agencyno      ,NULL AS requestno      ,NULL AS areacode      ,CASE WHEN t.agencyno = '" + pi_dsagencyno + "' THEN nvl(ds2.childnetno, nvl(ds.childnetno, t.netno)) ELSE t.netno END AS netno" + "      ,NULL AS marketioflag " + "      ,t.bk_tradeaccount AS tradeaccount " + "      ,NULL AS bk_region_of_trd " + "      ,NULL AS sk_agency_of_trd " + "      ,CASE WHEN t.agencyno = '" + pi_dsagencyno + "' THEN nvl(ds2.sk_agency, ds.sk_agency) END AS sk_agency_of_ds_acct " + "      ,ds.dk_capitalmode AS capitalmode " + "      ,cast(t.open_date as decimal(8,0)) AS sk_date " + "      ,t.open_date AS open_date_of_trdacct " + "      ,NULL AS bk_region " + "      ,'" + pi_dsagencyno + "' AS ds_agencyno " + "  FROM " + pi_schema + ".agrm_tradeacco_reg t " + " LEFT JOIN (SELECT bk_tradeaccount, " + "  agencyno, MAX(sk_agency) AS sk_agency, MAX(dk_capitalmode) AS dk_capitalmode, MAX(childnetno) AS childnetno " + " FROM " + pi_schema + ".agrm_tradeaccount_ds " + " GROUP BY bk_tradeaccount, agencyno) ds " + "  ON ds.bk_tradeaccount = t.bk_tradeaccount " + "  AND ds.agencyno = t.agencyno " + " LEFT JOIN (SELECT DISTINCT bk_channelno AS childnetno, sk_agency " + " FROM " + pi_schema + ".ip_agency WHERE agencyno = '" + pi_dsagencyno + "' AND dk_is_virtual_chnl = 'N' AND hiear_level <> 1) ds2 " + "  ON ds2.childnetno = t.childnetno " + "WHERE t.dk_system_of_sdata = '" + pi_srcsys + "' " + "  AND (t.batchno = " + pi_batchno + " OR CAST(1 AS STRING)= '" + pi_isAll + "' )";
        System.out.println("================sql_data====================");
        System.out.println(sql_data);
        System.out.println("=================sql_data===================");
        Dataset<Row> ipdf = hc.sql(sql_data);
        String sql_rule = "select t1.ag_split_ruleno               ,t1.agencyno               ,trim(t1.rule_name) as rule_name               ,trim(t1.rule_exp1) as rule_exp1               ,trim(t1.rule_exp2) as rule_exp2               ,trim(t1.rule_exp3) as rule_exp3               ,trim(t1.rule_exp4) as rule_exp4               ,trim(t1.rule_exp5) as rule_exp5               ,t1.priority               ,t1.effective_from               ,t1.effective_to           from " + pi_schema + ".ip_agency_split_param t1 " + "         where t1.dk_enabled_flag = 'Y' order by t1.priority";
        ...
        final Row[] rules = (Row[]) rdf.orderBy("priority", new String[0]).collect();
        Row[] rs = (Row[]) ddf.collect();
        final Map<String, List<Row>> map = new HashMap<>();
        List<Row> ls = null;
        String key = null;
        ...
        JavaRDD<Row> w = ipdf.toJavaRDD().map(new Function<Row, Row>() {
            ...
            public Row call(Row r) throws Exception {
                ...
                String agencyno = (String) r.getAs("agencyno");
                String requestno = (String) r.getAs("requestno");
                String tradeaccount = (String) r.getAs("tradeaccount");
                String bk_region = "N" + r.getAs("bk_region");
                String areacode = (String) r.getAs("areacode");
                String netno = (String) r.getAs("netno");
                String ds_agencyno = (String) r.getAs("ds_agencyno");
                String capitalmode = (String) r.getAs("capitalmode");
                String bk_region_of_trd = (String) r.getAs("bk_region_of_trd");
                String marketioflag = (String) r.getAs("marketioflag");
                String field = null;
                ...
                for (int i = 0; i < rules.length; i++) {
                    ...
                    if ("C_L".equals(rulename)) {
                        ...
                        if (!"9".equals(marketioflag) && !"*".equals(agencyno))
                            continue;
                        if ("9".equals(marketioflag) && "*".equals(agencyno) && "S".equals(rulename))
                            continue;
                    }
                    rule1 = (String) rules[i].getAs("rule_exp1");
                    if (rule1 != null) {
                    ...
                    rule2 = (String) rules[i].getAs("rule_exp2");
                    if (rule2 != null) {
                        ...
                        if (!"$$".equals(field.substring(0, (field.length() <= 2) ? field.length() : 2)) && field.length() >= start) {
                            if (agencyno.equals(ds_agencyno) && capitalmode != null) {
                                ...
                                if (!agencyno.equals(ds_agencyno)) {
                                    agcode2 = field.substring(start, (field.length() <= len) ? field.length() : len);
                                } else {
                                    continue;
                                }
                    ...
                    rule3 = (String) rules[i].getAs("rule_exp3");
                    if (rule3 != null) {
                    ...
                    rule4 = (String) rules[i].getAs("rule_exp4");
                    if (rule4 != null) {
                    ...
                    bk_agency = TradeaccoOpenAgencySplit.f_gen_bk_agency(agcode1, agcode2, agcode3, agcode4);
                    sk_agency = TradeaccoOpenAgencySplit.findSkAgency(sk_date, bk_agency, map);
                    if (sk_agency != null)
                        break;
                    continue;
                }
                if (sk_agency == null) {
                    bk_agency = TradeaccoOpenAgencySplit.f_gen_bk_agency(agencyno, "", "", "");
                    sk_agency = TradeaccoOpenAgencySplit.findSkAgency(sk_date, bk_agency, map);
                }
                return RowFactory.create(new Object[]{r
                        .getDecimal(0), sk_agency, rule_id, pi_srcsys});  // this.val$pi_srcsys
            }
        });
        StructType schema = hc.sql("select cast(0 as decimal(12,0)) as src_id,cast(0 as decimal(12,0)) as sk_agency,cast(null as string) as rule_id,cast(null as string) as srcsys").schema();
        System.out.println(schema + "." + pi_tgtTableName);
        hc.createDataFrame(w, schema)
                .write()
                .format("parquet").partitionBy(new String[]{"srcsys"}).mode(SaveMode.Overwrite)
                .saveAsTable(pi_schema + "." + pi_tgtTableName);
        hc.close();
    }
    ...
    static BigDecimal findSkAgency(int sk_date, String bk_agency, Map<String, List<Row>> map) {
        ...
        if (ls != null)
            for (int i = 0; i < ls.size(); i++) {
                dr = ls.get(i);
                if (sk_date >= Integer.parseInt(dr.getAs("effective_from").toString()) && sk_date <=
                        Integer.parseInt(dr.getAs("effective_to").toString())) {

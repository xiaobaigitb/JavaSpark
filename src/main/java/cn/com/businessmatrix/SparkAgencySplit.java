...
public class SparkAgencySplit {
    ...
    public static void main(String[] args) {
        ...
        //Modified By LuoGong,20220517：增加库名参数
        ...
        String sql_data =
                "SELECT trd_st_trx_serialno " +
                        ...
                        "  FROM " + schema_ods + ".mid_trd_ta_saletran t " +
        ...
        //有的要求有网点的，不走邮编，即使网点不全也不走邮编。
        String sql_mapp = "select t.bk_agency, t.sk_agency, t.effective_from, t.effective_to  from " + schema_ods + ".ip_agency_mapp t where t.dk_is_valid='Y'";
        ...
        JavaRDD<Row> w = ipdf.toJavaRDD().map(new Function<Row, Row>() {
            ...
            public Row call(Row r) throws Exception {
                ...
                for (int i = 0; i < rules.length; i++) {
                    // Modified By Luogong,20221020:规则有效期未生效修复
                    if (!(sk_date >= Integer.parseInt(rules[i].getAs("effective_from").toString())
                            && sk_date <= Integer.parseInt(rules[i].getAs("effective_to").toString())
                    ))
                        continue;
                    if (
                        ...
                        continue;
                    String v_agencyno = rules[i].getAs("agencyno");
                    ...
                    if ("T_L".equals(rulename)) {
                        ...
                        } else if (!"9".equals(marketioflag) && !"*".equals(v_agencyno)) {
                            continue;
                        } else if ("9".equals(marketioflag) && "*".equals(v_agencyno) && "S".equals(rulename)) {
                            continue;
                        }
                        ...
                        if (rule2 != null) {
                            ...
                            if ((!"$$".equals(field.substring(0, field.length() <= 2 ? field.length() : 2))) && field.length() >= start) {
                                if (v_agencyno.equals(ds_agencyno) && capitalmode != null) {
                                    agcode2 = field.substring(start, field.length() <= len ? field.length() : len);
                                } else if (v_agencyno.equals(ds_agencyno) && capitalmode == null) {
                                    if (sk_agency_of_ds_acct != null) {
                                        sk_agency = sk_agency_of_ds_acct;
                                        break;
                                    } else
                                        continue;
                                } else if (!v_agencyno.equals(ds_agencyno)) {
                                    agcode2 = field.substring(start, field.length() <= len ? field.length() : len);
                                } else {
                                    continue;
                                }
                            ...
                            } else if ("$$capitalmode".equals(field) && v_agencyno.equals(ds_agencyno) && capitalmode != null && capitalmode.length() >= start) {
        ...
        StructType schema = hc.sql("select cast(0 as decimal(12,0)) as src_id"
                ...
                .schema();
        hc.createDataFrame(w, schema).write().format("parquet").partitionBy("srcsys").mode(SaveMode.Overwrite).saveAsTable(pi_tgtTableName);
        hc.close();
    }
    ...
    static BigDecimal findSkAgency(int sk_date, String bk_agency, Map<String, List<Row>> map) {
        ...
        Row dr = null;
        BigDecimal sk_agency = null;
        //Modified By LuoGong,20220517：返回null出错，默认-1
        BigDecimal DefaultVal = BigDecimal.valueOf((int) -1);
        if (ls != null) {
